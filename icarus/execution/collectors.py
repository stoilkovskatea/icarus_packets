...
__all__ = [
    ...
    'DummyCollector',
    'LatencySessionsCollector',
    'CacheHitRatioSessionsCollector'
           ]
...
class LinkLoadCollector(DataCollector):
    ...
    def results(self):
        ...
        return Tree({'MEAN_INTERNAL':     mean_load_int,
                     'MEAN_EXT0ERNAL':     mean_load_ext,
                     'PER_LINK_INTERNAL': link_loads_int,
                     'PER_LINK_EXTERNAL': link_loads_ext})


@register_data_collector('LATENCY_SESSIONS')
class LatencySessionsCollector(DataCollector):
    ...
    def request_hop(self, u, v, main_path=True):
        if main_path:
            self.sess_latency[self.current_session] += self.view.link_delay(u, v)
...
@register_data_collector('CACHE_HIT_RATIO_SESSIONS')
class CacheHitRatioSessionsCollector(DataCollector):
