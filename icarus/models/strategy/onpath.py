...
from icarus.models.classes import Event
...
__all__ = [
       ...
       'RandomBernoulli',
       'RandomChoice',
       'LeaveCopyEverywhereUpdated'
           ]


@register_strategy('LCE_UPDATED')
class LeaveCopyEverywhereUpdated(Strategy):
    ...
    def __init__(self, view, controller, **kwargs):
        super(LeaveCopyEverywhereUpdated, self).__init__(view, controller)
    ...
    def process_event(self, time, packet, type, log, session_id):
        ...
        if type == 'interest':
            ...
            if packet.current_node != source:
                next_hop = packet.current_hop + 1
                next_node = path[next_hop]
                self.controller.forward_request_hop(packet.current_node, next_node)

                time_updated = time + self.view.link_delay(packet.current_node, next_node)
            ...
            if serving_node != packet.receiver:

                packet.serving_node = serving_node
                packet.current_node = next_node
                packet.current_hop = next_hop
            ...
            if packet.current_node == packet.receiver:
                ...
                path = list(reversed(self.view.shortest_path(packet.receiver, packet.serving_node)))
                next_hop = packet.current_hop + 1
                next_node = path[next_hop]
                self.controller.forward_content_hop(packet.current_node, next_node)
                time_updated = time + self.view.link_delay(packet.current_node, next_node)
