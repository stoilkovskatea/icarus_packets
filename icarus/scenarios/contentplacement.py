...
def uniform_content_placement(topology, contents, seed=None):
    ...
    if (all(isinstance(n, int) for n in contents)):
        for c in contents:
            s = random.choice(source_nodes)
            content_placement[s].add(c)
        ...
        while t < len(contents):
            n_chunks = contents[t].chunks
            current_source_node = random.choice(source_nodes)

            for i in range(t, t + n_chunks):
                content_placement[current_source_node].add(contents[i])
            t += n_chunks
