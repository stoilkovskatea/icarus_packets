...
from icarus.models.classes import Content
from icarus.models.classes import Event
from icarus.models.classes import Packet
...
@register_workload('STATIONARY_UPDATED')
class StationaryUpdated(object):
    '''Represents a content as a multiple objects, while keeping track of the link_delay '''

    def __init__(self, topology, n_contents, alpha, chunks_per_content, beta=0, rate=1.0,
        ...
        self.content_objects = [[Content(i, j) for j in range(self.n_chunks[i])] for i in range(self.n_contents)]
    ...
    def __iter__(self):
        ...
        while req_counter < self.n_warmup + self.n_measured:
            ...
            if (t_total >= t_content):  # if download of current content interrupted with new request
                ...
                event = {'packet':event_dispatched.packet,
